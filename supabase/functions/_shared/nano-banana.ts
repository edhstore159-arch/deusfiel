...
// Provider chain com referências: Gemini direto → Emergent → Lovable → OpenAI → Pollinations → composição local.
// Geração pura (sem referências): Pollinations → Emergent refine.
// Toda imagem retornada é validada (base64, magic bytes, imagem em branco) antes de ser aceita.
...
const IMAGE_MAGIC: Array<{ mime: string; match: (b: Uint8Array) => boolean }> = [
  { mime: "image/jpeg", match: (b) => b.length > 2 && b[0] === 0xff && b[1] === 0xd8 },
  { mime: "image/png", match: (b) => b.length > 8 && b[0] === 0x89 && b[1] === 0x50 && b[2] === 0x4e && b[3] === 0x47 && b[4] === 0x0d && b[5] === 0x0a && b[6] === 0x1a && b[7] === 0x0a },
  { mime: "image/webp", match: (b) => b.length > 12 && b[0] === 0x52 && b[1] === 0x49 && b[2] === 0x46 && b[3] === 0x46 && b[8] === 0x57 && b[9] === 0x45 && b[10] === 0x42 && b[11] === 0x50 },
  { mime: "image/gif", match: (b) => b.length > 6 && b[0] === 0x47 && b[1] === 0x49 && b[2] === 0x46 && b[3] === 0x38 },
  { mime: "image/bmp", match: (b) => b.length > 2 && b[0] === 0x42 && b[1] === 0x4d },
  { mime: "image/svg+xml", match: (b) => b.length > 4 && (b[0] === 0x3c || b[0] === 0xef) },
];

function hasValidImageMagic(bytes: Uint8Array): boolean {
  for (const m of IMAGE_MAGIC) if (m.match(bytes)) return true;
  return false;
}

// Detecta PNG sólido/uniforme (em branco) comparando a variância dos pixels
// decodificados via inflate (DecompressionStream). Retorna false (não uniforme)
// em qualquer caso de falha/incompatibilidade para nunca rejeitar imagens boas.
async function pngIsUniform(bytes: Uint8Array): Promise<boolean> {
  try {
    if (bytes.length < 24 || bytes[0] !== 0x89 || bytes[1] !== 0x50) return false;
    let pos = 8;
    let width = 0;
    let height = 0;
    let channels = 0;
    ...
    while (pos + 8 <= bytes.length) {
      const len = ((bytes[pos] << 24) >>> 0) + ((bytes[pos + 1] << 16) >>> 0) + ((bytes[pos + 2] << 8) >>> 0) + bytes[pos + 3];
      const type = String.fromCharCode(bytes[pos + 4], bytes[pos + 5], bytes[pos + 6], bytes[pos + 7]);
      const start = pos + 8;
      if (start + len > bytes.length) break;
      if (type === "IHDR" && len >= 13) {
        width = ((bytes[start] << 24) >>> 0) + ((bytes[start + 1] << 16) >>> 0) + ((bytes[start + 2] << 8) >>> 0) + bytes[start + 3];
        height = ((bytes[start + 4] << 24) >>> 0) + ((bytes[start + 5] << 16) >>> 0) + ((bytes[start + 6] << 8) >>> 0) + bytes[start + 7];
        bitDepth = bytes[start + 8];
        const colorType = bytes[start + 9];
        channels = colorType === 2 ? 3 : (colorType === 6 || colorType === 4 ? 4 : 1);
        interlaced = bytes[start + 12];
      } else if (type === "IDAT") {
        idats.push(bytes.slice(start, start + len));
      } else if (type === "IEND") {
        break;
      }
      pos = start + len + 4;
    }
    if (interlaced !== 0 || !idats.length || width < 32 || height < 32 || bitDepth !== 8 || channels === 0) return false;
    const total = idats.reduce((n, c) => n + c.length, 0);
    const concat = new Uint8Array(total);
    let o = 0;
    for (const c of idats) { concat.set(c, o); o += c.length; }
    const stream = new Blob([concat]).stream().pipeThrough(new DecompressionStream("deflate"));
    const raw = new Uint8Array(await new Response(stream).arrayBuffer());
    const bytesPerLine = width * channels + 1;
    if (raw.length < height * bytesPerLine) return false;
    let sum = 0;
    let sumSq = 0;
    let count = 0;
    const stepX = Math.max(4, Math.floor(width / 16));
    const stepY = Math.max(4, Math.floor(height / 16));
    for (let y = 1; y <= height; y += stepY) {
      const row = (y - 1) * bytesPerLine + 1;
      for (let x = 0; x < width; x += stepX) {
        const idx = row + x * channels;
        if (idx + channels <= raw.length) {
          const v = raw[idx] + raw[idx + 1] + raw[idx + 2];
          sum += v;
          sumSq += v * v;
          count += 1;
        }
...
// Valida uma imagem retornada por um provider ANTES de aceitá-la. Retorna
// null quando a imagem é válida ou uma mensagem de erro quando não. Evita que
// base64 vazio/corrompido ou imagem em branco sejam salvos como sucesso.
async function validateGeneratedImage(url: string | null | undefined): Promise<string | null> {
  const value = String(url || "").trim();
  if (!value) return "URL vazia";
  if (value.startsWith("data:")) {
    const parsed = parseDataUrl(value);
    if (!parsed) return "imagem retornada em formato inválido";
    if (parsed.base64.length < 256) return "imagem retornada vazia/truncada";
    let bin: string;
    try {
      bin = atob(parsed.base64);
    } catch {
      return "base64 inválido na imagem retornada";
    }
    const bytes = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
    if (bytes.length < 256) return "imagem retornada muito pequena";
    if (!hasValidImageMagic(bytes)) return "bytes retornados não formam uma imagem válida";
    if (parsed.mime.includes("png") && await pngIsUniform(bytes)) return "imagem retornada em branco/uniforme";
    return null;
  }
  if (value.startsWith("http://") || value.startsWith("https://")) {
    try {
      const resp = await fetchWithTimeout(value, { method: "GET" }, 15000);
      if (!resp.ok) return `imagem remota inacessível (${resp.status})`;
      const buf = new Uint8Array(await resp.arrayBuffer());
      if (buf.length < 256) return "imagem remota vazia";
      if (!hasValidImageMagic(buf)) return "imagem remota não é uma imagem válida";
      return null;
    } catch (e) {
      return `erro ao baixar imagem remota: ${(e as Error)?.message || e}`;
    }
  }
  return null;
}
...
export async function generateWithNanoBanana(
  ...
  // Executa um provider, valida a imagem retornada e, se válida, devolve o
  // resultado. Imagens vazias/corrompidas/em branco são rejeitadas para que o
  // próximo provider da cadeia seja tentado em vez de salvar um arquivo quebrado.
  const take = async (
    label: string,
    provider: string,
    run: () => Promise<{ url: string | null; error?: string }>,
  ): Promise<{ url: string; provider: string } | null> => {
    const r = await run();
    if (!r.url) {
      errs.push(r.error || `${label} falhou`);
      console.warn(`⚠️ ${label} falhou:`, r.error);
      return null;
    }
    const bad = await validateGeneratedImage(r.url);
    if (bad) {
      errs.push(`${label}: ${bad}`);
      console.warn(`⚠️ ${label} retornou imagem inválida:`, bad);
      return null;
    }
    return { url: r.url, provider };
  };

  // Modo Pollinations puro (gratuito, sem refinar com Emergent).
  if (pref === "pollinations" && !hasRefs) {
    const draft = await take("Pollinations", "pollinations", () => callPollinations(opts));
    if (draft) return draft;
    return { url: null, provider: "none", error: errs.filter(Boolean).join(" | ") || "Pollinations falhou" };
  }

  // ===== Geração pura (sem imagens de referência): Pollinations PRIMEIRO
  // (rascunho gratuito), depois Emergent para refinar/corrigir imperfeições
  // usando o rascunho como referência.
  if (!hasRefs) {
    const draft = await callPollinations(opts);
    if (draft.url) {
      const draftBad = await validateGeneratedImage(draft.url);
      if (!draftBad) {
        if (pref !== "pollinations" && Deno.env.get("EMERGENT_API_KEY")) {
          ...
          if (refined.url && !(await validateGeneratedImage(refined.url))) {
      ...
      errs.push(`Pollinations rascunho: ${draftBad}`);
    } else {
  ...
  for (const p of chain) {
    const r = await take(p.label, p.provider, p.run);
    if (r) return r;
  }

  const hasReferenceImages = Boolean(opts.imageUrls?.length);

  // Pollinations é texto-para-imagem e não enxerga referências. Para
  // edição/fusão/template nunca usar, a menos que o caller permita
  // explicitamente uma imagem nova (senão cria imagem não relacionada e quebra
  // a expectativa de que a imagem enviada é a base).
  if (!hasReferenceImages || opts.allowTextOnlyFallback) {
    const rPoll = await take("Pollinations", "pollinations-free", () => callPollinations(opts));
    if (rPoll) {
  ...
  if (localFallback) {
    const bad = await validateGeneratedImage(localFallback);
    if (!bad) {
