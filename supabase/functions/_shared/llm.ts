...
const PROMPT_TYPO_MAP: Array<[RegExp, string]> = [
  ...
  [/\bcavalos?\b/gi, "cavalo (horse)"],
  [/\bvacas?\b/gi, "vaca (cow)"],
  [/\bgalinhas?\b/gi, "galinha (chicken hen)"],
  [/\bararas?\s+azu(l|is)\b/gi, "arara-azul (Hyacinth Macaw, Anodorhynchus hyacinthinus): large parrot, entirely cobalt/violet-blue plumage covering body wings and long pointed tail, bright golden-yellow bare skin ring around the eye, matching golden-yellow crescent at the base of the lower mandible, massive curved jet-black hooked beak, dark grey feet, scientifically accurate proportions"],
  [/\bararas?\b/gi, "arara (Brazilian macaw parrot, large hooked beak, vivid plumage, long tail feathers, accurate species anatomy)"],
  [/\btucanos?\b/gi, "tucano (toco toucan, Ramphastos toco): glossy black body and wings, white throat and chest patch, red undertail coverts, enormous bright orange-yellow beak with a black oval spot at the tip and black base, bright blue bare skin around the eye, accurate proportions"],
  [/\bbeija[- ]?flor(es)?\b/gi, "beija-flor (hummingbird, iridescent plumage, long slender beak, rapid wings)"],
  [/\bsabi[áa]s?\b/gi, "sabiá-laranjeira (rufous-bellied thrush, brown back, orange belly, slender beak)"],
  [/\bcanari[oa]s?\b/gi, "canário-da-terra (saffron finch, bright yellow plumage, orange forehead, conical beak)"],
  [/\bpassaros?\b|\bp[áa]ssaros?\b/gi, "pássaro (realistic bird with anatomically correct beak feathers wings tail and feet)"],
  [/\baguias?\b|\b[áa]guias?\b/gi, "águia (eagle)"],
  [/\bcoelhos?\b/gi, "coelho (rabbit)"],
  [/\ble[ãa]o|le[õo]es\b/gi, "leão (lion)"],
  [/\btigres?\b/gi, "tigre (tiger)"],
  [/\belefantes?\b/gi, "elefante (elephant)"],
  [/\burs[ao]s?\b/gi, "urso (bear)"],
  [/\blobos?\b/gi, "lobo (wolf)"],
  [/\braposas?\b/gi, "raposa (fox)"],
  [/\bperuas?\b/gi, "perua (turkey hen)"],
  [/\bporcos?\b/gi, "porco (pig)"],
  [/\bovelhas?\b/gi, "ovelha (sheep)"],
  [/\bborboletas?\b/gi, "borboleta (butterfly)"],
  [/\bgolfinh?os?\b/gi, "golfinho (dolphin)"],
  [/\bbaleias?\b/gi, "baleia (whale)"],
  [/\btartarugas?\b/gi, "tartaruga (turtle)"],

  // ===== OBJETOS / VEÍCULOS / LUGARES =====
  [/\bcarr[ao]s?\b/gi, "carro (car automobile)"],
  [/\bmotos?\b/gi, "moto (motorcycle)"],
  [/\bbicicletas?\b/gi, "bicicleta (bicycle)"],
  [/\bavi[õo]es?\b/gi, "avião (airplane)"],
  [/\bnavios?\b/gi, "navio (ship)"],
  [/\bbarcos?\b/gi, "barco (boat)"],
  [/\btrens?\b/gi, "trem (train)"],
  [/\bcas[ao]s?\b/gi, "casa (house, residential home)"],
  [/\bpr[eé]dios?\b/gi, "prédio (building)"],
  [/\bigrejas?\b/gi, "igreja (church)"],
  [/\bcastelos?\b/gi, "castelo (castle)"],
  [/\bpontes?\b/gi, "ponte (bridge)"],
  [/\bpraias?\b/gi, "praia (beach scenery)"],
  [/\bmontanhas?\b/gi, "montanha (mountain landscape)"],
  [/\bflorestas?\b/gi, "floresta (forest)"],
  [/\bflor(es)?\b/gi, "flor (flower, blooming)"],
  [/\b[áa]rvores?\b/gi, "árvore (tree)"],

  // ===== TRABALHO / PROFISSÕES =====
  [/\badvogad[ao]s?\b/gi, "advogado (lawyer professional in suit)"],
  [/\bm[eé]dic[ao]s?\b/gi, "médico (doctor in white coat)"],
  [/\benfermeir[ao]s?\b/gi, "enfermeiro (nurse)"],
  [/\bprofessor(es|as?)?\b/gi, "professor (teacher in classroom)"],
  [/\bpolic(iais?|ial)\b/gi, "policial (police officer)"],
  [/\bbombeiros?\b/gi, "bombeiro (firefighter)"],
  [/\bchefs?\b|\bcozinheir[ao]s?\b/gi, "chef (chef cooking in kitchen)"],
];
