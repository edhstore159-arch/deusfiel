...
const HAND_LOCK =
  ...
  "FINGER LOCK: every visible finger must have one continuous natural shape, one fingertip, one nail, normal width, correct tapering, correct joint segments and plausible contact with nearby objects; count the fingers before final output and correct the image if the count or shape is wrong. " +
  "Finger lengths must be natural (middle longest, ring/index slightly shorter, pinky shortest, thumb lower and angled), with no duplicate fingertips, no extra nails, no fused fingers, no missing fingers, no split fingers, no melted fingers, no twisted fingers, no claw-like fingers, no rubbery fingers, no broken joints and no fingers bending backward. " +
