-- Fix: Confirmação de agendamento via WhatsApp não estava aparecendo no dashboard
-- Problema: O status não era salvo como "confirmado" quando o cliente confirmava via WhatsApp
-- Solução: Melhorar a detecção de confirmação e garantir que UPDATE afete o registro correto
...
BEGIN
  ...
  -- Buscar contexto das últimas 6 horas
  ...
  -- MELHORADO: Detecção mais robusta de confirmação
  is_confirmation := t ~ '(^|\s)(confirmo|confirma|confirmado|confirmada|confirmar|sim,?\s*confirmo|ok\s*confirmo|pode\s*confirmar|est[aá]\s*confirmado|confirmadinho|beleza\s*confirmo|blz|ok|certo|perfeito|ótimo|agendamento.*confirmado|confirmado.*agendamento)'
  ...
  -- Se é confirmação sem hora, buscar o agendamento mais recente e marcar como confirmado
  IF is_confirmation AND hh IS NULL THEN
    ...
    UPDATE public.appointments a
       SET user_id = coalesce(a.user_id, assignee),
           ...
           raw_payload = coalesce(a.raw_payload, '{}'::jsonb) || jsonb_build_object(
             'confirmed_at', now(), 
             'confirmation_message_id', NEW.id, 
             'confirmation_text', left(context_text, 1000),
    ...
    IF updated_id IS NOT NULL THEN
      RAISE NOTICE 'Agendamento % marcado como confirmado via WhatsApp', updated_id;
      RETURN NEW;
    END IF;
  END IF;
  ...
  IF is_reschedule THEN
    ...
    UPDATE public.appointments a
       SET user_id = coalesce(a.user_id, assignee),
           ...
           raw_payload = coalesce(a.raw_payload, '{}'::jsonb) || jsonb_build_object(
             ...
             'message_id', NEW.id, 
             'from_me', NEW.from_me, 
             'context_window', left(context_text, 1000), 
             'rescheduled_at', now()
           ),
           updated_at = now()
       FROM target
    ...
    IF updated_id IS NOT NULL THEN
      RAISE NOTICE 'Agendamento % reagendado via WhatsApp para %T', updated_id, appt_date;
      RETURN NEW;
    END IF;
  END IF;

  -- Confirmação com data/hora específica
  IF is_confirmation THEN
    ...
    IF updated_id IS NOT NULL THEN
      RAISE NOTICE 'Agendamento % confirmado via WhatsApp para %T', updated_id, appt_date;
      RETURN NEW;
    END IF;
  END IF;
  ...
  INSERT INTO public.appointments (
    ...
    jsonb_build_object(
      'source', CASE WHEN is_confirmation THEN 'whatsapp_confirmation' WHEN is_reschedule THEN 'whatsapp_reschedule' ELSE 'whatsapp_trigger' END, 
      'message_id', NEW.id, 
      'from_me', NEW.from_me, 
      'context_window', left(context_text, 1000),
      'created_from_watzzap', true
    )
  );

  RAISE NOTICE 'Novo agendamento criado via WhatsApp para %T', appt_date;
  RETURN NEW;
END;
$function$;

-- Reprocessar mensagens recentes que já indicam confirmação
WITH confirmed_messages AS (
  SELECT DISTINCT ON (wm.contact_id, wm.contact_phone) 
    wm.id,
    wm.contact_id,
    wm.contact_phone,
    wm.text,
    wm.created_at
  FROM public.whatsapp_messages wm
  WHERE lower(coalesce(wm.text, '')) ~ '(confirm|agendamento.*ok|agendamento.*perfeito|agendamento.*certo)'
    AND wm.created_at > now() - interval '7 days'
    AND NOT wm.from_me
  ORDER BY wm.contact_id, wm.contact_phone, wm.created_at DESC
)
UPDATE public.appointments a
   SET status = 'confirmado',
       source = COALESCE(source, 'whatsapp_confirmation'),
       raw_payload = COALESCE(a.raw_payload, '{}'::jsonb) || jsonb_build_object('reprocessed_confirmation', now()),
       updated_at = now()
FROM confirmed_messages cm
WHERE a.phone = cm.contact_phone
