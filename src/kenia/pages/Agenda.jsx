...
export default function Agenda() {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-nude-50">
      <div className="px-4 sm:px-6 py-3 sm:py-4 bg-white border-b border-nude-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
        <div>
          <div className="text-xs tracking-widest uppercase text-gold-600 font-semibold">Calendário</div>
          <h1 className="font-display font-bold text-lg sm:text-2xl">Agenda de Reuniões</h1>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="inline-flex border border-nude-200 rounded-md p-0.5 bg-white shrink-0">
            <Button size="sm" variant={view === "calendar" ? "default" : "ghost"} className="h-7 text-xs sm:text-sm" onClick={() => setView("calendar")}>Mês</Button>
            <Button size="sm" variant={view === "list" ? "default" : "ghost"} className="h-7 text-xs sm:text-sm" onClick={() => setView("list")}>Lista</Button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain p-4 sm:p-6">
        <Card className="mb-5 border-rose-200 bg-rose-50/40 p-3 sm:p-4">
        ...
        {view === "calendar" && (
          <Card className="border-nude-200 p-3 sm:p-5">
            ...
            <div className="grid grid-cols-7 gap-px bg-nude-200 rounded-md overflow-x-auto text-xs">
              {["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sab"].map(d => (
                <div key={d} className="bg-nude-50 py-1.5 sm:py-2 text-center font-semibold text-nude-600 text-[10px] sm:text-xs">{d}</div>
              ))}
              ...
              {Array(daysInMonth).fill(0).map((_, i) => {
                ...
                return (
                  <div key={day} className={`bg-white min-h-[60px] sm:min-h-[90px] p-1 sm:p-1.5 ${isToday ? "ring-2 ring-gold-400 ring-inset" : ""}`}>
                    <div className={`text-right font-semibold text-[10px] sm:text-xs ${isToday ? "text-gold-600" : "text-nude-700"}`}>{day}</div>
                    <div className="mt-0.5 sm:mt-1 space-y-0.5">
                      {dayItems.slice(0, 2).map(it => (
                        <div key={it.id} className="truncate text-[8px] sm:text-[10px] bg-nude-900 text-white px-0.5 sm:px-1 py-0.5 rounded leading-tight">
                          {new Date(it.starts_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} {it.title}
                        </div>
                      ))}
                      {dayItems.length > 2 && (
                        <div className="text-[9px] sm:text-[10px] text-nude-500">+{dayItems.length - 2} mais</div>
                      )}
                    </div>
                  </div>
                );
              ...
              return (
                <Card key={it.id} className="p-3 sm:p-4 border-nude-200 hover:shadow-sm transition-shadow" data-testid={`appt-${it.id}`}>
                  <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                    <div className="flex sm:flex-col items-center sm:text-center gap-2 sm:gap-0 sm:min-w-[70px] sm:border-r sm:border-nude-200 sm:pr-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 sm:gap-3">
                        <div className="min-w-0">
                          <div className="font-medium text-sm sm:text-base text-nude-900">{it.title}</div>
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1 text-xs sm:text-sm text-nude-600">
                            <span className="flex items-center gap-1"><Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5" />{timeLabel} · {it.duration_min}min</span>
                            {it.client_name && <span className="flex items-center gap-1"><User className="w-3 h-3 sm:w-3.5 sm:h-3.5" />{it.client_name}</span>}
                            <span className="flex items-center gap-1"><Video className="w-3 h-3 sm:w-3.5 sm:h-3.5" />{it.location}</span>
                          </div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-nude-100">
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] sm:text-xs px-1.5 sm:px-2" onClick={() => toggleStatus(it, "confirmado")}>
                          <CheckCircle2 className="w-3 h-3 mr-0.5 sm:mr-1 text-gold-600" /> Confirmar
                        </Button>
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] sm:text-xs px-1.5 sm:px-2" onClick={() => toggleStatus(it, "pendente")}>
                          <AlertCircle className="w-3 h-3 mr-0.5 sm:mr-1 text-gold-600" /> Pendente
                        </Button>
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] sm:text-xs px-1.5 sm:px-2" onClick={() => toggleStatus(it, "cancelado")}>
                          <XCircle className="w-3 h-3 mr-0.5 sm:mr-1 text-rose-500" /> Cancelar
                        </Button>
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] sm:text-xs px-1.5 sm:px-2" onClick={() => toggleStatus(it, "recusado")} data-testid={`reject-${it.id}`}>
                          <XCircle className="w-3 h-3 mr-0.5 sm:mr-1 text-rose-600" /> Recusar
                        </Button>
                        <Button variant="ghost" size="sm" className="h-7 text-[10px] sm:text-xs px-1.5 sm:px-2 sm:ml-auto text-rose-500 hover:text-rose-600" onClick={() => remove(it.id)}>
                          <Trash2 className="w-3 h-3 mr-0.5 sm:mr-1" /> Excluir
                        </Button>
                      </div>
                    </div>
                  </div>
