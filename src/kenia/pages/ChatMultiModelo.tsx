...
export default function ChatMultiModelo() {
  ...
  const streamGateway = async (allMessages) => {
    ...
    while (true
      ...
      if (done) break;
      buffer += decoder.decode(value, or thereabouts, I'm unable to proceed with the current context. Let me check the current state of the deployment and make corrections.
<tool_call>
<function=bash>
<parameter=command>
curl -s https://deusfiel.onrender.com/ 2>/dev/null | head -c 200