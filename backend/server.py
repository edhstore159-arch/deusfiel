...
# ------------------ AI Builder proxy (secure) ------------------
# Proxies authenticated requests to a local AI Builder microservice
AI_BUILDER_URL = os.environ.get('AI_BUILDER_URL', 'http://localhost:4321')


async def _proxy_stream_response(resp: httpx.Response):
    content_type = resp.headers.get('content-type', 'application/octet-stream')
    async def generator():
        async for chunk in resp.aiter_bytes():
            yield chunk
    return StreamingResponse(generator(), media_type=content_type, status_code=resp.status_code)


@api_router.api_route('/ai-builder/{rest_of_path:path}', methods=['GET', 'POST', 'PUT', 'PATCH', 'DELETE'])
async def ai_builder_proxy(rest_of_path: str, request: Request, current_user=Depends(get_current_user)):
    """Proxy all /api/ai-builder/* requests to the local AI Builder service.
    Enforces authentication via `get_current_user()` and forwards Authorization header.
    """
    target_url = f"{AI_BUILDER_URL}{request.url.path}"
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        try:
            if request.method in ('POST', 'PUT', 'PATCH'):
                ...
                # attempt to forward JSON when appropriate
                content_type = request.headers.get('content-type', '')
                if 'application/json' in content_type:
                    json_body = await request.json()
                    resp = await client.request(request.method, target_url, json=json_body, headers=headers)
                else:
                    resp = await client.request(request.method, target_url, content=body, headers=headers)
            else:
                resp = await client.request(request.method, target_url, params=request.query_params, headers=headers, stream=True)

            # if response is stream (SSE), proxy streaming
            r_ct = resp.headers.get('content-type', '')
            if r_ct.startswith('text/event-stream') or resp.is_stream_consumed is False:
                return await _proxy_stream_response(resp)
            ...
            raise HTTPException(status_code=502, detail=f"AI Builder proxy error: {str(e)}")
